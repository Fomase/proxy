...
SearchServer::SearchServer(const std::string& stop_words_text)
    ...
    bool SearchServer::IsValidWord(const std::string& word, bool isMinus) {
        std::size_t last_ch = word.size() - 1;
        if (isMinus && word[0] == '-') {
            return false;
        }
        if (word[last_ch] == '-') {
    ...
    std::vector<std::string> SearchServer::SplitIntoWordsNoStop(const std::string& text) const {
        ...
        for (const std::string& word : SplitIntoWords(text)) {
            if (!IsValidWord(word)) {
                throw std::invalid_argument("Некорректное содержание в списке слов документа"s); //заходит спецсимвол в бар...
            }
            if (!IsStopWord(word)) {
